...
{ "stops": [
  { "id": "41590-20000",     // 조인 키: {시군구코드}-{ARS번호}
    ...
    "boardingsPerDay": 1357.42 }   // 실측 일평균 승차(평일 기준). 없으면 생략 가능
]}
```

> `boardingsPerDay` 는 목록 항목에도 실립니다(`05_load.py` 의 `stops.json`).
> 지도의 정류장 점 크기와 노선 경유 순서 카드의 툴팁이 이 값을 씁니다.
> **없으면 프론트는 "자료 없음" 으로 적고 점 크기는 균일(3.4px)로 돌아갑니다** —
> 값이 없다고 `0` 으로 표시하면 실측 0 과 구분이 안 되기 때문입니다.
> 시간대별 상세는 `/stops/{id}/profile` 의 `summary` 를 계속 쓰세요.
