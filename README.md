...
| **관리자 콘솔** | `admin.html` | 파라미터·격자 판정을 담당자가 직접 조정하고, 무엇이·언제·왜 바뀌었는지 남깁니다 |
| **AI 보고서** | 두 화면 상단 버튼 | 현재 분석·시나리오를 근거로 검토 보고서 초안을 만들어 **한글(.rtf)·엑셀(.xlsx)** 로 저장합니다 |
| **AI 도우미** | 세 화면 공용 채팅 서랍 | 화면 질문에 답하고 대화로 화면을 옮겨 줍니다. 보고서 모달 안에서는 같은 UI 로 초안을 지시대로 고쳐 씁니다 |

**분석 대시보드** — 평일/주말 토글 · 격자 미스매칭 지도 · 이 격자를 지나는 노선 · 노선 조정
우선순위 Top 10 · 수요–공급 4분면 · 정류장 시간대 프로파일 · 권역별 요약

**정류장 시간대 프로파일이 있어야 하는 이유** — 격자 단위 분석만 있으면 "그래서 실제
정류장은 어떤데?"에 답을 못 합니다. 격자를 클릭하면 최근접 정류장 프로파일로 자동
전환되므로 거시(격자) ↔ 미시(정류장)를 오갈 수 있습니다.

**정책 시뮬레이션** — 평일/주말 토글(배치를 이미 올려 둔 상태에서도 오갈 수 있습니다) ·
배치 시뮬레이터(수단을 고르고 격자를 클릭해 배치, 영역 지정 가능, 모바일에서는 [빼기]
버튼으로 손가락으로도 제거 가능) · AI 추천 배치안(정류장·똑버스·증편 중 쓸 수단을 골라
제한 가능) · 시나리오(저장·불러오기·비교·파일 내보내기/가져오기·공유 링크) · 시간대별
사각지대 격자 기준선 대비 · 수단별 소요액 · 정책 판단 요약

**관리자 콘솔** — 배차 영향력·비용/예산·모델 상수·기준선 상수 파라미터 조정(근거·기본값과
함께 표시, "저장 즉시 반영"과 "재계산 필요"를 구분) · 격자 판정(사분면·수단·우선순위 점수)
수정 · 원본 데이터 재계산 · CSV 업로드 · 변경 이력(누가·언제·왜·이전값→새값) · 저장 위치를
로컬 초안/실제 DB 중 선택.

처음 화면을 열면 **스포트라이트 투어**가 지도부터 관리자 콘솔·AI 도우미까지 실제 화면을
어둠막으로 감싸 가리키며 6단계로 안내합니다 — 별도 설명 문서 대신 화면 자체가 다음
행동을 가리키게 하려는 선택입니다.

지도는 SVG 로 직접 그리고 그 뒤에 카카오맵 배경을 한 겹 깝니다.
확대하면 건물·지명이 드러나 "이 격자 안에 실제로 무엇이 있는지" 확인할 수 있습니다.

**작은 화면에서도 씁니다.** 1차 사용자(행정 담당자)는 기본적으로 사무실 모니터로 보지만,
현장에서 폰으로 확인하는 경우도 있어 320~768px 구간을 실측하며 터치 타깃(44px 이상)과
버튼 레이아웃을 다듬었습니다 — 시민 대상 반응형까지 챙긴 것은 아닙니다.
...
│       ├── core.js           공용 유틸(포맷·툴팁·테마·좌표투영·SVG 헬퍼·온보딩 투어)
│       ├── map.js            격자 지도 컴포넌트 (두 화면 공용 · 카카오 배경 정합 포함)
│       ├── chat.js           AI 채팅 (도움 모드·보고서 수정 모드 공용, api.js 를 거침)
...
| [백엔드 docs/API_SPEC.md](https://github.com/2026-AIHwaseong-project/hwaseong-dashboard-backend/blob/main/docs/API_SPEC.md) | 엔드포인트 13개 계약 **(정본)** |
