...
단가는 아직 **가정값**이라(실제 사업비 미확정) 화면·보고서에 "가정값" 표시가 자동으로 붙습니다.
...
단가가 확정되면 **금액은 백엔드** `meta.cost`(`analysis/05_load.py`)에서 고치고,
**"가정값" 배지는 프론트** `assets/js/config.js` 의 `COST[].confirmed` 를 `true` 로
바꿔야 내려갑니다. 서버 응답에는 단가의 가정값 플래그가 없어서
(`meta.assumptions` 에는 `busTripRate`·`minFreqPerHour` 뿐), 배지는 프론트 설정만 보고
켜집니다. 엑셀 보고서의 '사업비·내용연수' 문구는 `assets/js/report.js` 에 하드코딩돼
있어 같이 손봐야 합니다.
